 ...
 */
@Slf4j
public class OrderAppealCancelHandler implements OrderOperate {

    private OrderStatusCache orderStatusCache = OrderStatusCache.getInstance();

    @Override
    public void pay(Order order) {
        throw new UnsupportedOperationException("申诉取消状态的订单无法支付");
    }

    @Override
    public void cancel(Order order) {
        throw new UnsupportedOperationException("申诉取消状态的无法取消");

    }

    @Override
    public void appeal(Order order) {
        log.debug("申诉取消状态的订单发起申诉, order info = {}", JSONObject.toJSONString(order));
    ...
    public void release(Order order) {
        throw new UnsupportedOperationException("申诉取消状态的订单无法放行");
    }

    @Override
    public OrderStatusEnum getCurrentStatus(Order order) {
        return OrderStatusEnum.APPEAL_CANCEL;
    }
}
